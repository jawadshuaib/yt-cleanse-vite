...
const Dashboard: React.FC<DashboardProps> = ({ accessToken, onLogout }) => {
  ...
  const runAnalysis = useCallback(async () => {
    try {
      ...
      // Debug: log a few history items and their channel IDs
      // console.log('Sample history:', history.slice(0, 5));
      ...
      if (hasApiKey && channelsToAnalyze.length > 0) {
        ...
        const analysisResult = await analyzeSubscriptionsWithGemini(
          channelsToAnalyze,
          history,
        );
        const channelMap = new Map(channelsToAnalyze.map((c) => [c.id, c]));

        analysisResult.keep.forEach((id) => {
          const channel = channelMap.get(id);
          if (channel) categorized.keep.push(channel);
        });
        analysisResult.review.forEach((id) => {
          const channel = channelMap.get(id);
          if (channel) categorized.review.push(channel);
        });
        analysisResult.unsubscribe.forEach((id) => {
          const channel = channelMap.get(id);
          if (channel) categorized.unsubscribe.push(channel);
        });
      } else {
        if (channelsToAnalyze.length > 0) {
          ...
          console.warn(
            'Gemini API key not found or no channels to analyze. Falling back to basic local analysis.',
          );
